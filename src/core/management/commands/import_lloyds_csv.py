...
class Command(BaseCommand):
    help = "Imports Lloyds Bank transactions from CSV"

    def add_arguments(self, parser):
        parser.add_argument(
            'csv_path',
            help='absolute path to the CSV file with the Lloyds transactions',
    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic(), csvfile:
            ...
            LloydsService.truncate()
            try:
                LloydsService.import_csv(account_name, csv_reader)
            except ValueError:
                raise CommandError(
